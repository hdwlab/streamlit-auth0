...
def getVerifiedSubFromToken(token, domain, audience, unsafe_url_flag=False):
    ...
    if rsa_key:
        try:
            payload = jwt.decode(
                token, rsa_key, algorithms=["RS256"], audience=audience, issuer=domain + "/"
            )
...
def login_button(clientId, domain, key=None, **kwargs):
    ...
    user_info = _login_button(
        client_id=clientId, domain=domain, audience=audience, key=key, default=0
    )
    if not user_info:
        return None
    elif isAuth(
        response=user_info, domain=domain, audience=audience, unsafe_url_flag=unsafe_url_flag
    ):
        return user_info
    else:
...
def isAuth(response, domain, audience, unsafe_url_flag=False):
    return (
        getVerifiedSubFromToken(
            ...
            domain=domain,
            audience=audience,
            unsafe_url_flag=unsafe_url_flag,
        )
        == response["sub"]
    )
